...
def create_realistic_test_graph(n_channels: int = 50) -> tuple[CuvisPipeline, dict]:
    ...
    pipeline.connect(
        pca1.explained_variance_ratio, var_metric_1.explained_variance_ratio
    )

    var_metric_2 = ExplainedVarianceMetric()
    pipeline.connect(
        pca2.explained_variance_ratio, var_metric_2.explained_variance_ratio
    )
...
def test_gpu_acceleration():
    ...
    metric_nodes_warmup = [
        node
        for node in graph_cpu_warmup.nodes()
        if isinstance(node, ExplainedVarianceMetric)
    ]
    ...
    loss_nodes = [
        node for node in graph_cpu.nodes() if isinstance(node, OrthogonalityLoss)
    ]
    metric_nodes = [
        node for node in graph_cpu.nodes() if isinstance(node, ExplainedVarianceMetric)
    ]
    if loss_nodes:
    ...
    metric_nodes_gpu_warmup = [
        node
        for node in graph_gpu_warmup.nodes()
        if isinstance(node, ExplainedVarianceMetric)
    ]
    ...
    loss_nodes_gpu = [
        node for node in graph_gpu.nodes() if isinstance(node, OrthogonalityLoss)
    ]
    metric_nodes_gpu = [
...
def test_gpu_batch_size_scaling():
    ...
    for batch_size in batch_sizes:
        ...
        metric_nodes_batch_warmup = [
            node
            for node in graph_warmup.nodes()
            if isinstance(node, ExplainedVarianceMetric)
        ]
        ...
        loss_nodes_batch = [
            node for node in graph.nodes() if isinstance(node, OrthogonalityLoss)
        ]
        metric_nodes_batch = [
