...
- **`files` / `file_indices` selectors compare source paths normalized** (`os.path.normcase(os.path.normpath(...))`): a selector authored with backslashes or a different drive-letter case now matches the module's enumerated sources instead of silently selecting 0 samples on Windows. Comparison-only — `SampleRef.uid` derivation is untouched, so frozen splits and hashes stay stable.
