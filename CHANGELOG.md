...
- **Gated weights are provisioned out-of-band, not downloaded by the child.** The run child executes untrusted plugin code, so the spawner keeps `HF_TOKEN` / `HUGGINGFACE_HUB_TOKEN` stripped and runs the child offline against the pre-provisioned cache; the token never reaches plugin code. Fetch a gated weight (e.g. SAM3) ahead of time with `download-model` (token supplied in that trusted context), and the child then loads it offline. `tests/integration/test_sandbox_seams.py` locks this policy in.
