...
- A `LoadPipeline` whose plugins the session's child runtime was not composed for replaces the child instead of failing inside it. `ensure_child_for_session` reused a live child for every later load, so after a SAM3 `_view` preset (env `[sam3]`, no `cuvis_ai`) a dinomaly pipeline in the same session died with `No module named 'cuvis_ai'` until the session closed; the CuvisNEXT pipeline switch keeps one session on purpose, so users saw a stale session until they restarted the server. The bridge now resolves the plugins on every load and asks `child_can_serve`: a pipeline whose plugins are a subset of the child's, manifest for manifest, and whose data module is none or the one the env was composed with, reuses the warm child; anything else composes the new env first (a compose failure leaves the old child and its pipeline untouched), retires the old child through the one teardown path `SessionManager.retire_child` (graceful, then kill, exit confirmed; a survivor raises `ChildStillRunning` and the old child stays attached), then spawns the replacement, so the old child's GPU memory is back before the new model loads. `forward_load_pipeline` and `forward_restore_train_run` hold the session's new re-entrant `child_lock` from that decision through the forwarded call and `close_session` takes it too, so competing loads on one session serialise and a close cannot leave an orphan child or lease; a session closed while its child was being prepared answers `NOT_FOUND` with the fresh child stopped. A request that still reaches a child the parent itself stopped (`ChildHandle.retired_by_parent`, set only for a live child the parent stops; a child that died on its own keeps its crash status, trailers and preserved logs) is answered `FAILED_PRECONDITION` "stopped by the server", not as a crash: no logs preserved, no crash trailers. Compose and spawn failures (`UvRunnerError`, the runtime-project and spawner `RuntimeError`s, an `InitializeSession` the child refuses or fails, an `OSError` from the lease write or the process start) answer `FAILED_PRECONDITION` with the cause instead of an undecorated `UNKNOWN`, so a client that retries transport failures on a fresh session shows the cause once; a compose failure leaves the old child and its pipeline serving, a spawn failure after the retire leaves the session childless until the next load. `ChildStillRunning` is `FAILED_PRECONDITION` too: the survivor stays attached and marked, `child_can_serve` refuses it, and the next load retries the stop (a kill that lands late is then not a crash). `close_session` waits at most `CLOSE_LOCK_GRACE_SECONDS` (5 s) for a load that holds the lock, then stops the attached child, so a forwarded call that hangs cannot hold the close or the server's shutdown; `forward_restore_train_run` re-checks the session under the lock like `forward_load_pipeline` and answers `NOT_FOUND` for a session a close took first. The per-session scratch tree is recorded before the spawn, so a failed spawn or a session closed during the spawn leaves no `cuvis_runtime_sessions/<id>` behind, and crash logs are preserved per child (session and endpoint), so a session's second crashed child gets its own copy. Plugin resolution now runs on every load, so a pipeline that used to slip through only because the warm child happened to hold the package is rejected as before a first load would be.
- Closing a session whose load is still composing or spawning no longer waits for the compose. `close_session` sets `SessionState.closing`, waits `CLOSE_LOCK_GRACE_SECONDS` for the child lock, stops the attached child, waits `CLOSE_LOCK_RETRY_SECONDS` more and then tears down without the lock; the load re-checks the flag after the compose (dropping the old child's scratch tree), after the spawn (stopping the fresh child, dropping its lease and tree) and before forwarding, and answers NOT_FOUND. The server's shutdown, which closes sessions one after another, is bounded the same way. A request that reaches a child the parent stopped, or finds no child while a load is replacing it, is answered ABORTED (repeat the request) instead of FAILED_PRECONDITION, which the desktop client shows once and never retries; a request with no child and no load in flight keeps FAILED_PRECONDITION. Compose and spawn failures are caught by class (`SpawnError`, `UvRunnerError`, `ComposerError`, `RuntimeProjectError`) instead of `RuntimeError`, so a `NotImplementedError` or `RecursionError` in the resolver reaches the servicer as a bug. `child_can_serve` compares the install identity (source, package name, data-module extras), so a plugin re-registered with regenerated capability metadata keeps the warm child.
