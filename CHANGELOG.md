...
- **Shared model cache for sandboxed runs + a `download-model` provisioning CLI.** New `orchestrator/model_cache.py` (`model_cache_dir()` / `model_cache_env()`) points the HuggingFace and torch caches at a shared `<cache_root>/model_cache`, exports `CUVIS_MODEL_CACHE_DIR`, and runs each child `HF_HUB_OFFLINE=1`; the run spawner injects these so weight-bearing nodes resolve from that shared cache instead of a wiped per-run HOME. New `download-model` CLI (`[hf]` extra) pre-fetches gated or public weights into that cache with sha validation, for offline or shipped installs. `composer.resolve_cache_root()` is now public.
