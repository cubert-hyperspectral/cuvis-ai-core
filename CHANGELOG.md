...
- **Shared model cache for sandboxed runs + a `download-model` provisioning CLI.** New `orchestrator/model_cache.py` (`model_cache_dir()` / `model_cache_env()`) points the HuggingFace and torch caches at a shared `<cache_root>/model_cache` and exports `CUVIS_MODEL_CACHE_DIR`; the run spawner injects these into each child so weight-bearing nodes resolve (and download once into) a shared cache instead of a wiped per-run HOME. New `download-model` CLI (`[hf]` extra) pre-fetches gated or public weights into that cache with sha validation, for offline or shipped installs. `composer.resolve_cache_root()` is now public.
- **The run spawner now forwards `HF_TOKEN` / `HUGGINGFACE_HUB_TOKEN` into the child** (previously stripped) so gated weights such as SAM3 download on first run. Accepted, tracked security debt: the child runs plugin code, so a follow-up moves gated downloads to a trusted provisioning pass and restores the strip. `tests/integration/test_sandbox_seams.py` encodes the new policy.
